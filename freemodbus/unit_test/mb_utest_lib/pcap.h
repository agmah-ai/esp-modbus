 ...
 *
 * @param[in] pcap pcap file handle created by `pcap_new_session()`
 * @param[in] pfile_header pointer to pcap file header structure
 * @return
 *      - ESP_OK: Read pcap file header successfully
 *      - ESP_ERR_INVALID_ARG: Read pcap file header failed because of invalid argument
 *      - ESP_FAIL: Read pcap file header failed
 */
 ...
 *
 * @param[in] pcap pcap file handle created by `pcap_new_session()`
 * @param[in/out] pindex pointer to index of the packet to get from file (if pindex = -1, returns the number of packets in the pcap file)
 * @param[in] payload pointer of the captured data buffer
 * @param[in] plength pointer to length of read data buffer
 * @param[in] seconds second of capture time
 * @param[in] microseconds microsecond of capture time
 * @return
 *      - ESP_OK: Write network packet into pcap file successfully
