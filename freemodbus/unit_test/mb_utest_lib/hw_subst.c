...
BOOL __wrap_xMBPortSerialWaitEvent(QueueHandle_t xQueueHandle, void* pxEvent, ULONG xTimeout)
{
    if (!pxEvent) {
        return FALSE;
    }
    ...
    uart_event_t* pxUartEvent = pxEvent;    
    err = ut_stream_wait_notification(STREAM_ID_INPUT, xTimeout, &event_length);
    if ((err == ESP_OK) && (event_length > 0)) {
        if (pxUartEvent) {
            pxUartEvent->type = UART_DATA;
            pxUartEvent->timeout_flag = true;
            pxUartEvent->size = event_length;
        }
