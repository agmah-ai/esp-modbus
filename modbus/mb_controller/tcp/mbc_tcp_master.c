...
static esp_err_t mbc_tcp_master_set_descriptor(void *ctx, const mb_parameter_descriptor_t *descriptor, const uint16_t num_elements)
{
    ...
    for (int idx = 0; idx < (num_elements); idx++, reg_ptr++) {
        ...
        paddr_info = mbm_port_tcp_find_slave_addr(mbm_controller_iface->mb_base->port_obj, reg_ptr->mb_slave_addr);
        MB_RETURN_ON_FALSE((paddr_info), ESP_ERR_INVALID_ARG, TAG, 
                            "mb missing IP address configuration for cid #%d, uid=%d.", reg_ptr->cid, reg_ptr->mb_slave_addr);
        ESP_LOGI(TAG, "mb found config for cid #%d, uid=%d.", reg_ptr->cid, reg_ptr->mb_slave_addr);
    }
...
static esp_err_t mbc_tcp_master_send_request(void *ctx, mb_param_request_t *request, void *data_ptr)
{
    ...
    if (mb_port_evt_res_take(mbm_controller_iface->mb_base->port_obj, pdMS_TO_TICKS(MB_MAX_RESP_DELAY))) { //pdMS_TO_TICKS(mbm_opts->comm_opts.tcp_opts.response_tout_ms)
        ...
        switch(mb_command) {
            ...
            case MB_FUNC_READ_COILS:
                mb_error = mbm_rq_read_coils(mbm_controller_iface->mb_base, (uint8_t)mb_slave_addr, (uint16_t)mb_offset,
                                                (uint16_t)mb_size , 
                                                pdMS_TO_TICKS(MB_MAX_RESP_DELAY));
                break;
#endif
...
static esp_err_t mbc_tcp_master_get_parameter(void *ctx, uint16_t cid, uint8_t *value, uint8_t *type)
{
    ...
    if ((error == ESP_OK) && (cid == reg_info.cid) && (request.slave_addr != MB_SLAVE_ADDR_PLACEHOLDER)) {
        mb_slave_addr_info_t *paddr_info = mbm_port_tcp_find_slave_addr(mbm_controller_iface->mb_base->port_obj, request.slave_addr);
        MB_RETURN_ON_FALSE((paddr_info), ESP_ERR_NOT_FOUND, TAG,
                            "mb can not send request for cid #%d with uid=%d.", reg_info.cid, request.slave_addr);
...
static esp_err_t mbc_tcp_master_get_parameter_with(void *ctx, uint16_t cid, uint8_t uid, uint8_t *value, uint8_t *type)
{
    ...
    if ((error == ESP_OK) && (cid == reg_info.cid)) {
        ...
        mb_slave_addr_info_t *paddr_info = mbm_port_tcp_find_slave_addr(mbm_controller_iface->mb_base->port_obj, uid);
        MB_RETURN_ON_FALSE((paddr_info), ESP_ERR_NOT_FOUND, TAG,
                            "mb can not send request for cid #%d with uid=%d.", reg_info.cid, uid);
        if (request.slave_addr != MB_SLAVE_ADDR_PLACEHOLDER) {
            ESP_LOGD(TAG, "%s: override uid %d = %d for cid(%u)",
                            __FUNCTION__, request.slave_addr, uid, (unsigned)reg_info.cid);
        }
...
static esp_err_t mbc_tcp_master_delete(void *ctx)
{
    ...
    MB_RETURN_ON_FALSE((mb_error == MB_ENOERR), ESP_ERR_INVALID_STATE, TAG,
                        "mb stack delete failure, returned (0x%x).", (uint16_t)mb_error);
...
esp_err_t mbc_tcp_master_create(mb_communication_info_t *config, void **ctx)
{
    ...
    MB_GOTO_ON_FALSE((ret == ESP_OK), ESP_ERR_INVALID_STATE, error, TAG, "mb create returns (0x%x).", (uint16_t)ret);
    ...
    if (tcp_opts.mode == MB_TCP) {
        err = mbm_tcp_create(&tcp_opts, &mbm_controller_iface->mb_base);
    }
    MB_GOTO_ON_FALSE((err == MB_ENOERR), ESP_ERR_INVALID_STATE, error, TAG, "mb create returns (0x%x).", (uint16_t)ret);
    ...
    mbm_port_tcp_set_conn_cb(mbm_controller_iface->mb_base->port_obj, 
                                &mbc_tcp_master_conn_done_cb, (void *)mbm_controller_iface);
