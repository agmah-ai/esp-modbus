        ...
        uint16_t ncoils, mb_reg_mode_enum_t mode)
{
    ...
    if ((num_coil_regs >= 1)
            && (coils_buf)
            && (ncoils == num_coil_regs)) {
        reg_index = (reg_addr % 8);
        switch (mode) {
            case MB_REG_WRITE:
                while (coils_cnt > 0) {
                    uint8_t ucResult = mb_util_get_bits((uint8_t *)coils_buf, reg_index, 1);
                    mb_util_set_bits(reg_buffer, reg_index - (reg_addr % 8) , 1, ucResult);
                    reg_index++;
                    coils_cnt--;
                }
                break;
            case MB_REG_READ:
                while (coils_cnt > 0) {
                    uint8_t ucResult = mb_util_get_bits(reg_buffer, reg_index - (reg_addr % 8), 1);
                    mb_util_set_bits((uint8_t *)coils_buf, reg_index, 1, ucResult);
                    reg_index++;
                    coils_cnt--;
                }
                break;
                                                ...
                                                uint16_t n_discrete)
{
    ...
    if ((num_discr_regs >= 1)
        ...
        while (num_reg > 1) {
            mb_util_set_bits(temp_discr_buf++, bit_index, 8, *reg_buffer++);
            num_reg--;
        }
        // last discrete
        ...
        if (n_discrete != 0) {
            mb_util_set_bits(temp_discr_buf, bit_index, n_discrete, *reg_buffer++);
        }
    } else {
        status = MB_ENOREG;
    }
