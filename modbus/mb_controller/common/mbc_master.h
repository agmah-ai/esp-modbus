...
// Set the maximum resource waiting time to 2sec, the actual time of resouce release
// will be dependent on response time set by timer + convertion time if the command is received
#define MB_MAX_RESP_DELAY (3000) // pdMS_TO_TICKS(mbm_opts->comm_opts.ser_opts.response_tout_ms)
 ...
 */
typedef struct _port_serial_opts mb_master_comm_info_t;
...
typedef esp_err_t (*iface_set_descriptor_fp)(void *, const mb_parameter_descriptor_t*, const uint16_t);     /*!< Interface set_descriptor method */
...
typedef struct {
    ...
    iface_set_descriptor_fp set_descriptor;         /*!< Interface set_descriptor method */
