...
static esp_err_t mbc_serial_slave_delete(void *ctx)
{
    ...
    if (mbs_iface->is_active || (status & MB_EVENT_STACK_STARTED)) {
        ...
        MB_RETURN_ON_FALSE((mbc_serial_slave_stop(ctx) == ESP_OK), 
    ...
    MB_RETURN_ON_FALSE((mb_error == MB_ENOERR), ESP_ERR_INVALID_STATE, TAG,
                        "mb stack close failure returned (0x%x).", (uint16_t)mb_error);
    free(mbs_iface); // free the memory allocated for options
    ctx = NULL;
    return ESP_OK;
}
...
static esp_err_t mbc_serial_slave_controller_create(void **ctx)
{
    ...
    MB_RETURN_ON_FALSE((mbs_controller_iface == NULL), ESP_ERR_INVALID_STATE,
                            TAG, "mb stack is not destroyed.");
...
esp_err_t mbc_serial_slave_create(mb_communication_info_t *config, void ** ctx)
{
    ...
    MB_RETURN_ON_FALSE(((pcomm_info->mode == MB_RTU) || (pcomm_info->mode == MB_ASCII)),
                        ESP_ERR_INVALID_ARG, TAG, "mb incorrect mode = (0x%x).",
                        (uint16_t)pcomm_info->mode);
    ...
    MB_RETURN_ON_FALSE((pcomm_info->slave_addr <= MB_ADDRESS_MAX),
                        ESP_ERR_INVALID_ARG, TAG, "mb wrong slave address = (0x%x).",
                        (uint16_t)pcomm_info->slave_addr);
    ...
    MB_GOTO_ON_FALSE((ret == ESP_OK), ESP_ERR_INVALID_STATE, error, TAG, 
                        "mb create returns (0x%x).", (uint16_t)ret);
    ...
    if (pcomm_info->mode == MB_RTU) {
        err = mbs_rtu_create(pcomm_info, &mbs_controller_iface->mb_base);
    } else if (pcomm_info->mode == MB_ASCII) {
        err = mbs_ascii_create(pcomm_info, &mbs_controller_iface->mb_base);
    }    
    MB_GOTO_ON_FALSE((err == MB_ENOERR), ESP_ERR_INVALID_STATE, error, TAG, 
                        "mb create returns (0x%x).", (uint16_t)err);
