...
mb_err_enum_t mb_port_evt_create(mb_port_base_t *inst)
{
    ...
    SPIN_LOCK_INIT(pevent->spin_lock);
...
{
    SPIN_LOCK_ENTER(inst->event_obj->spin_lock);
    inst->event_obj->curr_err_type = event;
    SPIN_LOCK_EXIT(inst->event_obj->spin_lock);
}

inline __attribute__((always_inline)) mb_err_event_t mb_port_evt_get_err_type(mb_port_base_t *inst)
{
    return inst->event_obj->curr_err_type;
}
...
{
    assert(inst->event_obj->event_hdl != NULL);
    ...
    if (xPortInIsrContext()) {
        ...
        if (result == pdPASS) {
            ...
            ESP_EARLY_LOGV(TAG, "%p, post message %x failure .", inst, temp_event.event);
            return false;
        }    
        if (high_prio_task_woken) {
            portYIELD_FROM_ISR();
        ...
        result = xQueueSend(inst->event_obj->event_hdl, (const void*)&temp_event, MB_EVENT_QUEUE_TIMEOUT);
        MB_RETURN_ON_FALSE((result == pdTRUE), false, TAG, "%p, post message failure.", inst);
    }
    return true;
}
...
bool mb_port_evt_res_take(mb_port_base_t *inst, uint32_t timeout)
{
    ...
    MB_RETURN_ON_FALSE((status == pdTRUE), false , TAG, "%p, mb resource take failure.", inst);
    ESP_LOGD(TAG, "%p, mb take resource, (%" PRIu32 " ticks).", inst, timeout);
    return true;
}
...
void mb_port_evt_res_release(mb_port_base_t *inst)
{
    ...
    if (status != pdTRUE) {
        ESP_LOGD(TAG,"%p, mb resource release.", inst);
    }
}
...
mb_err_enum_t mb_port_evt_wait_req_finish(mb_port_base_t *inst)
{
    ...
    if (rcv_event) {
        ESP_LOGD(TAG,"%p, %s: returned event = 0x%x", inst, __func__, rcv_event);
        if (!(rcv_event & MB_EVENT_REQ_MASK)) {
            // if we wait for certain event bits but get from poll subset
            ESP_LOGE(TAG,"%p, %s: incorrect event set = 0x%x", inst, __func__, (uint16_t)rcv_event);
        }
        ...
        ESP_LOGE(TAG,"%p, %s: incorrect event or timeout, rcv_event = 0x%x", inst, __func__, (uint16_t)bits);
        err_status = MB_ETIMEDOUT;
    }
    return err_status;
}
