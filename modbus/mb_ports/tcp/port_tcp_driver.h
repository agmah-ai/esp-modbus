...
#include "mdns.h"
//#warning "MB_MDNS_IS_INCLUDED included."
#endif
...
#define INVALID_FD              (-1)
#define MB_EVENT_TOUT           (100 / portTICK_PERIOD_MS)
...
#define MB_SELECT_WAIT_MS           (500)
#define MB_TCP_SEND_TIMEOUT_MS      (500)
#define MB_TCP_EVENT_LOOP_TICK_MS   (100)
...
typedef enum _mb_slave_state {
    MB_SOCK_STATE_UNDEF = 0x0000,
    MB_SOCK_STATE_CLOSED = 0x0001,
    MB_SOCK_STATE_READY,
...
typedef struct _slave_addr_info {
    uint16_t index;                     /*!< index of the address info */
    int fd;                             /*!< slave global FD for VFS (reserved) */
    const char *node_name_str;          /*!< node name string (host name of slave to resolve) */
    const char *ip_addr_str;            /*!< represents the IP address of the slave */
    mb_addr_type_t addr_type;           /*!< type of IP address */
    uint16_t slave_addr;                /*!< slave unit ID (UID) field for MBAP frame  */
    uint16_t port;                      /*!< slave port number */
    mb_comm_mode_t proto;               /*!< protocol type */
    mb_slave_state_t state;             /*!< slave state */
} mb_slave_addr_info_t;
...
typedef struct _mb_slave_info {
    ...
    mb_slave_addr_info_t addr_info;     /*!< slave address info structure*/
...
//     void* pinst;
//     mb_slave_addr_info_t addr_info;
...
typedef struct _port_driver {
    ...
    //LIST_HEAD(mb_slave_addr_info_, mb_slave_addr_entry_s) slave_list; /*!< Slave address information list */
 ...
 *          - ESP_OK on success
 */
esp_err_t mbm_intf_register(port_driver_t **config);

/**
 * @brief Unregister modbus driver
 *
 * @param ctx - pointer to driver interface structure
 * @param base_path Base path to the MODBUS virtual filesystem driver. Default path /dev/net/modbus/tcp is used when this paramenter is NULL.
 * @return esp_err_t
 *          - ESP_OK on success
 */
 ...
 *
 * @param slave_addr - modbus slave address of the slave
