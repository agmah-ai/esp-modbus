...
typedef struct
{
    ...
    uint8_t ptemp_buf[MB_SERIAL_BUF_SIZE];
    port_driver_t *pdriver;
} mbm_tcp_port_t;

/* ----------------------- Static variables & functions ----------------------*/
static const char *TAG = "mb_port.tcp.master";
static bool mbm_port_timer_expired(void *inst);
static void mbm_port_tcp_sync_event(void *inst, mb_sync_event_t sync_event);

extern int port_scan_addr_string(char *buffer, mb_slave_addr_info_t *pslave_info);
...
mb_err_enum_t mbm_port_tcp_create(mb_tcp_opts_t *tcp_opts, mb_port_base_t **port_obj)
{
    ...
    err = mbm_intf_register(&ptcp->pdriver);
    ...
    MB_GOTO_ON_FALSE((paddr_table && *paddr_table), MB_EILLSTATE, error, 
                        TAG, "mb tcp port driver registeration failed.");
    mb_slave_addr_info_t slave_address_info;
    int fd = 0;
    ...
    while(*paddr_table) {
        ...
        if (res > 0) {
            ESP_LOGW(TAG, "Config: %s, IP: %s, port: %d, slave_addr: %d, ip_ver: %s", 
                        ...
                        slave_address_info.slave_addr, (slave_address_info.addr_type == MB_IPV4 ? "IPV4" : "IPV6"));            
            fd = mbm_open(ptcp->pdriver, slave_address_info, 0);
            if (fd < 0) {
                ...
                //(void)mbm_port_tcp_register_slave(&(ptcp->base), &slave_address_info);
            }
        } else {
...
error:
    if (ptcp && ptcp->pdriver) {
        (void)mbm_intf_unregister(ptcp->pdriver, NULL);
...
void mbm_port_tcp_delete(mb_port_base_t *inst)
{
    ...
    esp_err_t err = mbm_intf_unregister(port_obj->pdriver, NULL);
    if (err != ESP_OK) {
...
void mbm_port_tcp_enable(mb_port_base_t *inst)
{
    ...
    if (!port_obj->pdriver->is_registered && !port_obj->pdriver) {
        err = mbm_intf_register(&port_obj->pdriver);
        MB_RETURN_ON_FALSE((err == ESP_OK), ;, TAG, "mb tcp port driver register failed.");
    }
    (void)mbm_start_task(port_obj->pdriver);
...
{
    mbm_tcp_port_t *port_obj = __containerof(inst, mbm_tcp_port_t, base);
    //mbm_stop_task(port_obj->pdriver);
    mbm_intf_unregister((port_driver_t *)port_obj->pdriver, NULL);
    port_obj->pdriver = NULL;
}
...
bool mbm_port_tcp_recv_data(mb_port_base_t *inst, uint8_t **ppframe, uint16_t *plength)
{
    ...
    size_t sz = mbm_read(port_obj->pdriver, pinfo->fd, port_obj->ptemp_buf, MB_SERIAL_BUF_SIZE);
    if (sz > MB_TCP_FUNC) {
...
bool mbm_port_tcp_send_data(mb_port_base_t *inst, uint8_t address, uint8_t *pframe, uint16_t length)
{
    ...
    mb_slave_info_t *pinfo = (mb_slave_info_t *)mbm_get_slave_info_from_addr(port_obj->pdriver, address);
    ...
    CRITICAL_SECTION(port_obj->base.lock) {
        if (pinfo && pframe) {
            ...
            pframe[MB_TCP_UID] = (uint8_t)(pinfo->addr_info.slave_addr);
        }
    }
    ...
    int write_length = mbm_write(port_obj->pdriver, pinfo->fd, pframe, length);
    if (write_length) {
        frame_sent = true;
    } else {
        ESP_LOGE(TAG, "mbm_write fail, returns %d.", write_length);
    }
    mb_port_tmr_respond_timeout_enable(inst);
...
{
    mbm_tcp_port_t *port_obj = __containerof(inst, mbm_tcp_port_t, base);
    mbm_set_cb(port_obj->pdriver, conn_fp, arg);
}
...
bool mbm_port_timer_expired(void *inst)
{
    ...
    if (mb_port_get_cur_tmr_mode(inst) == MB_TMODE_RESPOND_TIMEOUT) {
        // It is now to check solution. Todo: find proper mechanizm to sincronize to tout events        
...
{
    mbm_tcp_port_t *port_obj = __containerof(inst, mbm_tcp_port_t, base);
    mb_slave_addr_info_t *paddr_info = NULL;
    mb_slave_info_t *pinfo = mbm_get_slave_info_from_addr(port_obj->pdriver, slave_addr);
    if (pinfo && (MB_GET_SLAVE_STATE(pinfo) >= MB_SOCK_STATE_OPENED)) {
