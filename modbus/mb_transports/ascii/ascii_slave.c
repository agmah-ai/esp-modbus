...
mb_err_enum_t mbs_ascii_transp_create(mb_serial_opts_t *ser_opts, mb_trans_base_t **out_inst);
...
static bool mbs_ascii_transp_tmr_1s_expired(void *inst);
...
mb_err_enum_t mbs_ascii_transp_create(mb_serial_opts_t *ser_opts, mb_trans_base_t **out_inst)
{
    ...
    // Todo: move port creation to mb_object
    ...
    port_obj->cb.tmr_expired = mbs_ascii_transp_tmr_1s_expired;
...
static void mbs_ascii_transp_start(mb_trans_base_t *inst)
{
    ...
    CRITICAL_SECTION(inst->lock) {
        mb_port_ser_enable(inst->port_obj, true, false);
...
{
    CRITICAL_SECTION(inst->lock) {
        mb_port_ser_enable(inst->port_obj, false, false);
...
static mb_err_enum_t mbs_ascii_transp_receive(mb_trans_base_t *inst, uint8_t *prcv_addr, uint8_t **ppframe_buf, uint16_t *pbuf_len)
{
    ...
    uint16_t length = transp->rcv_buf_pos;
